...
def sync_project_timezones_task(self, project_client: Optional[ProjectClient] = None):
    ...
    try:
        ...
        # Enqueue while lock is still held so a failed ``delay()`` does not clear the lock first.
        # Chained run uses skip_sync_lock_check so it is not skipped by this same lock.
