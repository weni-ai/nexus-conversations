...
- `CONVERSATION_RESOLUTION_NAME`: Legacy resolution Lambda function name (flat `conversation` array payload). Used for projects listed in `CONVERSATION_RESOLUTION_LEGACY_PROJECTS`.
- `CONVERSATION_RESOLUTION_V2_NAME`: New resolution Lambda function name (nested `conversation.messages` payload). Default for all projects not in the legacy list.
- `CONVERSATION_RESOLUTION_LEGACY_PROJECTS`: Comma-separated project UUIDs that keep using the legacy resolution Lambda. New projects omit from this list to use V2 automatically.
- V2 resolution payloads include `user_rules` loaded from Nexus `GET /api/{project_uuid}/ai-resolution-criteria/` (via `NexusClient`). `model` is not set by conversations.
...
- `SQS_BILLING_QUEUE_URL`: Outbound FIFO queue for billing on **conversation close** (only after resolution bulk-update succeeds). Message body JSON: `channel_uuid`, `start_date` (UTC `Z`), `end_date` (UTC `Z`), `contact_urn`, `resolution` (string code), `uuid` (conversation PK). FIFO `MessageGroupId` uses `channel_uuid:contact_urn` when that value is valid for SQS and ≤128 chars; otherwise it falls back deterministically to a SHA-256-based value derived from `channel_uuid:contact_urn` (either a safe `channel_uuid:` prefix plus a digest suffix, or the digest alone when the prefix contains characters SQS rejects—`channel_uuid` is normally a UUID). Same safety model as the nexus-ai events producer. Omitted when unset.
