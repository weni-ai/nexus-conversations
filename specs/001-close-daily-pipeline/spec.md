...
| **Stage completed-at** | Timestamp set only when status is `done` or `skipped` (`close_{stage}_at`) |
| **Stage pending-at** | Timestamp set only while status is `pending` (`close_{stage}_pending_at`); drain stale clock |
...
|-------|---------------------|
| **1 — Foundation** | 18 stage columns (status/at/error/pending_at × 4 + 2 datalake event ats), outbox model, CheckConstraints, backfill, `ClosePipelineStateMachine`, tests; close runtime unchanged |
...
| **3 — Drain & harden** | Drain beat, stale pending reclaim via `*_pending_at`, metrics/Sentry per stage, selector lock/timeout tuning |
...
1. **Given** eligible In Progress conversations for a project day, **When** the project selector runs, **Then** it claims classify (`pending` + `pending_at`), sets close window end if needed, enqueues classify tasks, and does not invoke Lambdas or Billing inline.
2. **Given** a classify task, **When** it runs, **Then** it performs resolution only (not topics Lambda), commits classify + initializes topics/billing/datalake atomically, and enqueues topics, billing, and datalake (datalake may send only the classification event until topics finishes).
...
4. **Given** legacy all-`done` or never-claimed `NULL` stages, **When** drain runs, **Then** they are not selected for replay.
5. **Given** incomplete pipelines after classify, **When** operators query, **Then** they can list conversations with classify finished and billing not finished.
...
- **FR-002**: System MUST persist stage completed-at when status is `done` or `skipped`, MUST persist stage error when status is `failed`, and MUST persist `close_{stage}_pending_at` when (and only when) status is `pending`, for every stage.
- **FR-003**: System MUST reject illegal stage shapes and illegal precedence combinations at the persistence boundary via CheckConstraints + state machine (see data-model.md). Terminal resolution with all stages `NULL` remains DB-legal for out-of-band closes; close-daily MUST NOT produce that shape.
- **FR-004**: System MUST initialize topics, billing, and datalake statuses in the same commit that finishes classify successfully on the close-daily path (Shape C). Datalake MUST be initialized as `pending` (never `skipped` at Shape C in v1).
...
- **FR-016**: When topics is `skipped`, datalake MUST still publish the topics event using the existing bias/empty-metadata builder (parity with current close-daily). Setting `close_datalake_topics_at` without an external publish is forbidden.

### Key Entities

- **Conversation**: Existing entity; gains pipeline control-plane fields per stage; `resolution` remains business outcome.
- **Close pipeline stage state**: Per-stage status + completed-at + pending-at + error with strict shape rules.
