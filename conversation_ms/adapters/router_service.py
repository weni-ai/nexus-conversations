...
class MainConversationService:
    ...
    def _maybe_move_start_date_earlier_from_message(
        ...
        ``start_date``'s local date) and can break reuse of this row for later same-day
        messages; skip cross-day backdates.
        """
        ...
        try:
            ...
            if msg_date.in_timezone("UTC") >= conv_start:
                return
            if calendar_date_in_project_timezone(msg_date, tz_name) != calendar_date_in_project_timezone(
                conversation.start_date, tz_name
            ):
                return
            conversation.start_date = msg_date
    ...
    def _create_conversation(
        ...
        msg_date = pendulum.parse(msg_created_at)
        start_date = msg_date
        end_date = end_of_project_local_calendar_day_utc(msg_date, tz_name)
