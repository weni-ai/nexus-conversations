...
1. Read [spec.md](./spec.md) user stories + FR/SC + clarifications (`ClosePipelineRecord` 1:1, DAG, pending_at, billing skipped vs failed, **Session 2026-08-04 dead letter**, **Session 2026-08-05 outage formula + Redis**).
...
4. Implementation order: foundation → cutover → drain (prefer same release for foundation+cutover; drain ships **with** outage mode).
...
- **Billing outage mode** (see table below): evaluate each billing drain tick; persist `active` + `clear_streak` in Redis; while active may re-enqueue but MUST NOT increment billing reclaim / mark `dead`
...
| Redis outage key | `conversation_ms:close_pipeline:billing_outage` |
| Redis outage TTL (refresh/tick) | 86400 |
| Classify cohort ops target | 12h from claim (SC-016) |

### Outage open / clear (cheat sheet)
...
`rate` / `attempts` recalculate every tick; Redis keeps `active` + `clear_streak` across ticks/restarts.
...
- **Billing brownout**: read Redis `conversation_ms:close_pipeline:billing_outage` (`active`, `clear_streak`); set `CLOSE_PIPELINE_BILLING_OUTAGE_PAUSE=true` to force; bulk-reopen premature `dead` after recovery
