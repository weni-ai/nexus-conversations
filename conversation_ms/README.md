...
- `SQS_BILLING_QUEUE_URL`: Outbound FIFO queue for billing on **conversation close** (only after resolution bulk-update succeeds). Message body JSON: `channel_uuid`, `start_date` (UTC `Z`), `contact_urn`, `resolution` (string code), `uuid` (conversation PK). Omitted when unset.
